...
class NonLocalSparseAttention(nn.Module):
    ...
    def LSH(self, hash_buckets, x):   #hash桶
        #x: [N,H*W,C]
        N = x.shape[0]
        device = x.device
    ...
    def forward(self, input):
        N,_,H,W = input.shape
        x_embed = self.conv_match(input).view(N,-1,H*W).contiguous().permute(0,2,1)  #contiguous把tensor变成在内存中连续分布的形式
        ...
        hash_codes = hash_codes.detach()  #不计算梯度
