...
from utils.tools import reverse_patches
...
class Un(nn.Module):
    ...
    def forward(self,x):
        ...
        out = x3
        b,c,h,w = x3.shape
        p = self.reduce(torch.cat([x1,x2,x3],dim=1))  #16 96 32 32   #16 32 32 32
        out = self.attention(self.reduce(torch.cat([x1,x2,x3],dim=1))) #16 1024 288
        out = out.permute(0,2,1)
        out = reverse_patches(out, (h,w), (3,3), 1, 1) #16 32 32 32
        out = self.alise(out)
...
class ESRT(nn.Module):
    ...
    def forward(self, x1,x2 = None, test=False):
        # x1 = self.sub_mean(x1)
        x1 = self.head(x1)  #第一层卷积 32 32 32
        res2 = x1
        body_out = []
        for i in range(self.n_blocks):
            x1 = self.body[i](x1)
            body_out.append(x1)
        res1 = torch.cat(body_out,1) #32 32 32
        res1 = self.reduce(res1) #32 32 32

        
        x1 = self.tail(res1) #3 96 96
        x1 = self.up(res2) + x1 
        return x1
